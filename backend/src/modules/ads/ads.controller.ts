...
import { CreateAdDto, UpdateAdDto, AdResponseDto, UploadResponseDto } from './dto/ads.dto';
...
export class AdsController {
    ...
    @ApiResponse({ status: 201, description: 'File uploaded', type: UploadResponseDto })
    async uploadMedia(
        @UploadedFile(
            new ParseFilePipe({
                validators: [
                    new MaxFileSizeValidator({ maxSize: 10 * 1024 * 1024 }), // 10MB
                    new FileTypeValidator({ fileType: /(jpg|jpeg|png|gif|webp|mp4|webm|mov)$/ }),
                ],
            }),
        )
    ...
    async updateAd(
        ...
        const ad = await this.adsService.updateAd(businessId, adId, userId, dto, file);
        return {
            id: ad.id.toString(),
            title: ad.title,
            mediaUrl: ad.mediaUrl,
