...
import { TrackInteractionDto, ConnectWifiDto, TrackResponseDto, AnalyticsSummaryDto } from './dto/analytics.dto';
...
export class AnalyticsController {
    ...
    @ApiOperation({
        ...
        description: 'Track view or click interaction from captive portal. Non-blocking for optimal UX.'
    })
    @ApiResponse({ status: 200, description: 'Interaction tracked', type: TrackResponseDto })
    ...
    @ApiOperation({
        ...
        description: 'Log connection and return CTA redirect URL (Google Review deep-link)'
    })
    @ApiResponse({
        status: 200,
        ...
        schema: {
            properties: {
                ...
                redirectUrl: { type: 'string', example: 'https://g.page/r/abc123/review' },
            },
        },
    })
    async connectWifi(
    ...
    @ApiQuery({ name: 'startDate', description: 'Start date (ISO)', required: false })
    @ApiQuery({ name: 'endDate', description: 'End date (ISO)', required: false })
    @ApiResponse({ status: 200, description: 'Analytics summary', type: AnalyticsSummaryDto })
