...
import { ComplianceLog, ComplianceLogDocument } from './schemas/compliance-log.schema';
...
export class ComplianceService {
    ...
    async logSession(input: ComplianceLogInput): Promise<ComplianceLogDocument> {
        const log = new this.complianceModel({
            ...
            businessId: input.businessId ? new Types.ObjectId(input.businessId) : undefined,
     ...
     */
    async getLogsByMacAddress(macAddress: string, limit: number = 100): Promise<ComplianceLogDocument[]> {
     ...
     */
    async getLogsByPhone(phone: string, limit: number = 100): Promise<ComplianceLogDocument[]> {
        return this.complianceModel
            .find({ phone })
            .sort({ loginTime: -1 })
            .limit(limit);
     ...
     */
    async getLogsByBusiness(businessId: string, limit: number = 100): Promise<ComplianceLogDocument[]> {
        return this.complianceModel
    ...
    async getStats(): Promise<{
        ...
        const [
            ...
            this.complianceModel.distinct('phone').then(arr => arr.filter(p => p).length),
