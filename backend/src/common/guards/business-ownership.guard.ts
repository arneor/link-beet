...
import { AdminAccessLog, AdminAccessLogDocument } from '../../modules/admin/schemas/admin-access-log.schema';
...
export class BusinessOwnershipGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (user.type === 'admin' || user.role === 'super_admin') {
            ...
            this.logger.log(`Admin ${user.email} accessing business ${businessIdParam}`);
            return true;
        }
